...
  st.sidebar.header("Options")
  ...
  # Add a button for prediction at the top of sidebar
  ...
  if st.sidebar.button("Show Heatmap"):
    ...
    heatmap_data = df.groupby(['Latitude', 'Longitude', 'Tire_Brand']).size().reset_index(name='counts')

    # Add a heatmap layer to the map
    HeatMap(data=heatmap_data[['Latitude', 'Longitude', 'counts']].values.tolist(), radius=15).add_to(m)
